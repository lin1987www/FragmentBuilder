...
public final class HttpCookie implements Cloneable {
    private static final String TAG = "HttpCookie";
    /* ---------------- Fields -------------- */
    ...
    private String commentURL; // CommentURL="http URL" ... describes cookie's
    // use
    private boolean toDiscard; // Discard ... discard cookie unconditionally
    private String domain; // Domain=VALUE ... domain that sees cookie
    private long maxAge = MAX_AGE_UNSPECIFIED; // Max-Age=VALUE ... cookies
    // auto-expire
    private String path; // Path=VALUE ... URLs that see the cookie
    private String portlist; // Port[="portlist"] ... the port cookie may be
    // returned to
     ...
     * @throws IllegalArgumentException if the cookie name contains illegal characters or it is one
     *                                  of the tokens reserved for use by the cookie protocol
     ...
     * section 3.2.2 set-cookie2 syntax indicates that one header line may
     * contain more than one cookie definitions, so this is a static utility
     * method instead of another constructor.
     *
     * @param header a <tt>String</tt> specifying the set-cookie header. The header
     *               should start with "set-cookie", or "set-cookie2" token; or it
     *               should have no leading token at all.
     * @return a List of cookie parsed from header line string
     * @throws IllegalArgumentException if header string violates the cookie specification's syntax,
     *                                  or the cookie name contains llegal characters, or the cookie
     *                                  name is one of the tokens reserved for use by the cookie
     *                                  protocol
     * @throws NullPointerException     if the header string is <tt>null</tt>
     */
    public static List<HttpCookie> parse(String header) {
        return parse(header, true);
    }
    ...
    /**
     * Specifies a comment url that describes a cookie's purpose. The comment
     * url is useful if the browser presents the cookie to the user. Comment url
     * is RFC 2965 only.
     *
     * @param purpose a <code>String</code> specifying the comment url to display to
     *                the user
     * @see #getCommentURL
     */
     ...
     *
     * @return a <code>String</code> containing the comment url, or
     * <code>null</code> if none
     * @see #setCommentURL
     */
    ...
    /**
     * Specify the portlist of the cookie, which restricts the port(s) to which
     * a cookie may be sent back in a Cookie header.
     *
     * @param ports a <tt>String</tt> specify the port list, which is comma
     *              seperated series of digits
     * @see #getPortlist
     */
     ...
     *
     * @return a <tt>String</tt> contains the port list or <tt>null</tt> if none
     * @see #setPortlist
     */
     ...
     * <p/>
     * <p/>
     * The form of the domain name is specified by RFC 2965. A domain name
     ...
     *                this cookie is visible; form is according to RFC 2965
     * @see #getDomain
     */
     ...
     *               seconds; if zero, the cookie should be discarded immediately;
     ...
     * <p/>
     * <p/>
     * Consult RFC 2965 (available on the Internet) for more information on
     ...
     *
     * @param flag If <code>true</code>, the cookie can only be sent over a
     *             secure protocol like https. If <code>false</code>, it can be
     *             sent over any protocol.
     * @see #getSecure
     */
    ...
    /**
     * Returns <code>true</code> if sending this cookie should be restricted to
     * a secure protocol, or <code>false</code> if the it can be sent using any
     * protocol.
     *
     ...
     *          specification; 1 if the cookie should comply with RFC
     *          2965/2109
     ...
     * concept, some terminologies need to be defined first: <blockquote>
     * effective host name = hostname if host name contains dot<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
     * nbsp;&nbsp;&nbsp;&nbsp;&nbsp;or = hostname.local if not </blockquote>
     * <p/>
     ...
     * <li>The request-host is a HDN (not IP address) and has the form HD, where
     * D is the value of the Domain attribute, and H is a string that contains
     * one or more dots.</li>
     * </ul>
     * </blockquote>
     * <p/>
     ...
     * Examples: <blockquote>
     * <ul>
     * <li>A Set-Cookie2 from request-host y.x.foo.com for Domain=.foo.com would
     * be rejected, because H is y.x and contains a dot.</li>
     * <li>A Set-Cookie2 from request-host x.foo.com for Domain=.foo.com would
     * be accepted.</li>
     * <li>A Set-Cookie2 with Domain=.com or Domain=.com., will always be
     * rejected, because there is no embedded dot.</li>
     * <li>A Set-Cookie2 with Domain=ajax.com will be accepted, and the value
     * for Domain will be taken to be .ajax.com, because a dot gets prepended to
     * the value.</li>
    ...
    /**
     * Constructs a cookie header string representation of this cookie, which is
     * in the format defined by corresponding cookie specification, but without
     * the leading "Cookie:" token.
     *
     ...
     * <p/>
     * <p/>
     * The result is <tt>true</tt> only if two cookies come from same domain
     * (case-insensitive), have same name (case-insensitive), and have same path
     * (case-sensitive).
     *
     * @return <tt>true</tt> if 2 http cookies equal to each other; otherwise,
     * <tt>false</tt>
     */
    @Override
    public boolean equals(Object obj) {
        ...
        return equalsIgnoreCase(getName(), other.getName())
                ...
                && equalsIgnoreCase(getPath(), other.getPath());
    }

    /**
     * Return hash code of this http cookie. The result is the sum of hash code
     * value of three significant components of this cookie: name, domain, and
     * path. That is, the hash code is the value of the expression: <blockquote>
            ...
            if (index != -1) {
                ...
                if (retainHeader)
                    cookie = new HttpCookie(name,
                            stripOffSurroundingQuote(value), header);
                else
                    cookie = new HttpCookie(name,
    ...
    static {
        ...
        assignors.put("comment", new CookieAttributeAssignor() {
            public void assign(HttpCookie cookie, String attrName,
                               String attrValue) {
        ...
        assignors.put("expires", new CookieAttributeAssignor() { // Netscape
            // only
            public void assign(HttpCookie cookie, String attrName,
                               String attrValue) {
                if (cookie.getMaxAge() == MAX_AGE_UNSPECIFIED) {
                    cookie.setMaxAge(cookie
                            .expiryDate2DeltaSeconds(attrValue));
                }
                //  記錄過期日期字串
                cookie.setExpires(attrValue);
            }
        });
    }

    private static void assignAttribute(HttpCookie cookie, String attrName,
                                        String attrValue) {
        ...
        if (assignor != null) {
            ...
            Log.e(TAG, String.format(
                    "Ignore the attribute as per RFC 2965 [%s][%s]. from [%s]",
                    attrName, attrValue, cookie.toString()));
        }
    }
    ...
    // .setJavaNetHttpCookieAccess(new sun.misc.JavaNetHttpCookieAccess() {
     ...
     * comma; 2) but not the comma surrounding by double-quotes, which is the
     * comma inside port list or embeded URIs.
     *
